...
Metadata normalization now has an optional provenance-preserving LLM rephrase stage. When `OPENAI_API_KEY` is configured and `METADATA_REPHRASE_ENABLED` is not disabled, the graph asks the configured model to group CV content into concise factual sentences and keywords. Each returned item must reference an exact-enough source excerpt and valid page before it is accepted. If the call fails, returns malformed JSON, or fails provenance validation, the workflow falls back to deterministic section normalization and exposes a warning in the metadata response. Set `METADATA_REPHRASE_TIMEOUT_SECONDS`, `METADATA_REPHRASE_MAX_OUTPUT_TOKENS`, or `METADATA_REPHRASE_ENABLED=false` as needed.
...
The metadata review page displays the rephrased value, keywords, provenance, and normalization method. After confirmation, the Agent Communication panel polls `GET /api/workflows/{workflow_id}/discussion` with cache disabled and refreshes the conversation state at the server-provided interval. It currently reflects the existing handoff and pending graph phases; it does not fabricate future agent messages before those graph nodes exist.

### Phase 4 internet retrieval

Phase 4 adds `app/retrieval/` and `POST /api/retrieval/search`. The endpoint accepts a retrieval intent, target role, optional location/preparation period, source names, and a result limit. It returns generated queries, contradiction-query candidates, snippet-level search results, original-page retrieval results, page passages, warnings/errors, and an Evidence Ledger. It does not generate free-form recommendations.

The source registry is configurable through `RETRIEVAL_SOURCE_REGISTRY`; built-in policies cover Work24, Saramin, and JobKorea. Existing server scraping and Work24 functions are exposed through adapters. Search results are deduplicated by canonical URL, and evidence is only created after the original page is retrieved. Restricted, timed-out, inaccessible, expired, or low-quality pages are returned with explicit status or warnings and are not approved as evidence. `GET`/`POST` retrieval execution is not yet a graph research node; that belongs to the later Consulting phase.

### Phase 5 Consulting Agent

Phase 5 adds `app/consulting/` and `POST /api/consulting/analyze`. The Consulting Agent accepts `UserConfirmedMetadata`, preferred role, preparation period, configurable company/reference-case limits, and source selection. It reuses the retrieval pipeline, ranks opportunities with an inspectable deterministic score (role relevance, active status, experience match, skill match, freshness, and source quality), and keeps an optional semantic rank as a separate weighted component. Only original-page-backed evidence is eligible; snippets alone do not create approved market claims. Company-posting and market-requirement claims remain `PENDING` until the later Judge phase. Accepted-candidate cases are represented by a reserved interface policy and are not retrieved or asserted. The endpoint returns partial results and warnings when retrieval is incomplete, and it does not generate free-form recommendations.

### Phase 6 Supporting Agents

Phase 6 adds `app/supporting/` with four scoped interfaces: project/career experience, leadership/contribution, language/credential, and CV positioning/expression. `POST /api/supporting/analyze` activates only agents intersecting the selected gap categories and runs independent agents through a bounded thread pool. Each output is schema-validated at finding level and references confirmed metadata item IDs, claim IDs, and evidence IDs where available. `MISSING` is distinct from explicit `CONFIRMED_ABSENCE`; unsupported or unknown references are rejected by `POST /api/consulting/review`. The review interface returns `APPROVED`, `REVISION_REQUIRED`, `MORE_EVIDENCE_REQUIRED`, or `UNVERIFIABLE`. The implementation does not call an LLM Judge and does not create Planner, Calendar, readiness, or free-form final recommendation output.

### Phase 7 Judge and adaptive debate

Phase 7 adds `app/judge/` and `POST /api/judge/debate`. The reusable Judge interface evaluates one atomic claim at a time using only the supplied claim, supplied metadata context, and supplied evidence. It supports conservative verdicts for support, partial support, contradiction, ambiguity, stale evidence, insufficient source quality, and unverifiable claims. The OpenAI provider requests structured JSON without hidden reasoning; absent keys, provider failures, malformed JSON, or invalid evidence references fall back to the deterministic mock Judge. Bounded debate configuration limits rounds and retries; unresolved ambiguity becomes `UNVERIFIABLE`, while explicit contradiction remains `CONTRADICTED` with contradiction evidence IDs. Planner, Calendar, readiness classification, and final synthesis are not included.

### Phase 8 readiness classification

Phase 8 adds `app/readiness/` and `POST /api/readiness/classify`. The deterministic readiness policy computes target-role requirement coverage, critical gap count, evidence-backed experience strength, preparation-period feasibility, credential coverage, unresolved/contradictory claim counts, approved claim count, and stale/low-quality evidence count. `Stable` is gated strictly: market requirements must exist, all critical gaps and contradictory/unresolved claims must be absent, and every coverage/experience/feasibility/evidence condition must pass. The response contains structured indicators, policy reasons, limitations, confidence, and an explicit non-guarantee disclaimer. It does not generate recommendations, Planner tasks, Calendar events, or employment predictions.

The Planner portion adds `app/planner/` and `POST /api/planner/plan`. It accepts only approved finding/recommendation candidates and returns typed todo items plus proposal-only calendar records. External deadlines are preserved only when matching evidence is verified, fresh, sufficiently authoritative, and active. Expired opportunities are excluded from calendar eligibility; undated or suggested dates remain tentative/user-confirmation-required. `calendar_write_enabled` is always false in this phase, and no Google Calendar API is called.
